...
public class MainContentServiceImpl implements MainContentService {
	...
	public ArrayList<JinSunMiVo> getJinSunMi(TourBasicVo vo) {
		...
		ArrayList<JinsunmiTourIdxAvgTourScoreJoinVo> selectJinsunmi = tourMapper.selectJinSunMi(vo, startDate, endDate);
		
		System.out.println(selectJinsunmi);
		System.out.println(selectJinsunmi.size());
		if(selectJinsunmi.size() == 0) {
			...
			for(JinsunmiTourIdxAvgTourScoreJoinVo jinsunmi : selectJinsunmi) {
				//진선미의 tourInfo가져오기
				...
				JinSunMiVo jinsunmiInfo = new JinSunMiVo(jinsunmiMemberInfo,jinsunmiTourInfo,jinsunmi.getTourAvgScore());
				//list에 추가
	...
	public TourBasicVo getTourInfo(int tour_idx) {
		...
		System.out.println(tour.getLocation_idx());
		System.out.println(tour.getTour_idx());
		TourBasicVo tourContentData = tourMapper.selectTourByTourIdx(tour);
		System.out.println(tourContentData.getLocation_idx());
		System.out.println(tourContentData.getTour_idx());
