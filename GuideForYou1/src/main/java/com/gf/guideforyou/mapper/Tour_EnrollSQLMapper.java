...
import com.gf.guideforyou.vo.JinsunmiTourIdxAvgTourScoreJoinVo;
...
public interface Tour_EnrollSQLMapper {
	...
	@Insert("INSERT INTO GF_TOUR_ENROLLMENT VALUES(GF_TOUR_ENROLLMENT_SEQ.NEXTVAL,#{member_idx},#{location_idx},#{tour_tourspot},#{tour_max_number},"
			+ "#{tour_vehicle},#{tour_tourdate},#{tour_intro},#{tour_content},'n',SYSDATE,"
	...
	//Na 추가
		...
		@Update("UPDATE GF_TOUR_ENROLLMENT SET LOCATION_IDX=#{location_idx}, TOUR_TOURSPOT=#{tour_tourspot}, TOUR_MAX_NUMBER=#{tour_max_number}, TOUR_VEHICLE=#{tour_vehicle}, TOUR_TOURDATE=#{tour_tourdate}, TOUR_INTRO=#{tour_intro}, TOUR_CONTENT=#{tour_content}, TOUR_PHOTO1=#{tour_photo1 , jdbcType=VARCHAR},TOUR_PHOTO2=#{tour_photo2 , jdbcType=VARCHAR},TOUR_PHOTO3=#{tour_photo3 , jdbcType=VARCHAR},TOUR_PHOTO4=#{tour_photo4 , jdbcType=VARCHAR},TOUR_PHOTO5=#{tour_photo5 , jdbcType=VARCHAR}")
		public void updateTourContent(TourBasicVo vo);
		
		//jinsunmi 추출 ++한솔 추가
		@Select("SELECT * FROM (SELECT TOUR_IDX, ROUND(AVG(GUIDE_SCORE),1) FROM GF_GUIDE_EVALUATION WHERE TOUR_IDX IN (SELECT TOUR_IDX FROM GF_TOUR_ENROLLMENT WHERE MEMBER_IDX IN (SELECT MEMBER_IDX FROM GF_TOUR_ENROLLMENT WHERE LOCATION_IDX=#{para.location_idx} GROUP BY MEMBER_IDX) AND TOUR_TOURDATE <= #{endDate} AND TOUR_TOURDATE >= #{startDate}) GROUP BY TOUR_IDX ORDER BY ROUND(AVG(GUIDE_SCORE),1) DESC) WHERE ROWNUM <= 3")
		public ArrayList<JinsunmiTourIdxAvgTourScoreJoinVo> selectJinSunMi(@Param("para") TourBasicVo vo, @Param("startDate") String year, @Param("endDate") String month);
